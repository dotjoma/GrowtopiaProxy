...
public:
    ...
    void init() override
    {
        ...
        const auto web_port = static_cast<uint16_t>(core_->get_config().get<unsigned int>("web.port"));
        if (!server_.bind_to_port("0.0.0.0", web_port)) {
            spdlog::error("Failed to bind to port {}.", web_port);
            return;
        }

        spdlog::info("HTTP(s) server listening on port {}.", web_port);
